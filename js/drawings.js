...
//----------------------------------toggle pois--------------------------------------
function togglePOI(shape,forcetoggle=false){
 if(shape.visible==undefined){  
	shape.poiMarker = new google.maps.Marker({
   position: shape.LLcenter,
   title: shape.name,
   icon: shape.icon,
   map: map
  });

  var contentString = '<div id="content"><h1 id="infoWindowHeading" class="infoWindowHeading">' + shape.name +  
  '</h1><div id="infoWindowBodyContent" style="width: 480px;">' +
  ((shape.picture != "") ? ('<img class="infoWindowImages" src="' + shape.picture + '">') : ('<iframe title=' + shape.name + 
  ' width="100%" height="270" allowTransparency="true" mozallowfullscreen webkitallowfullscreen allowfullscreen style="background-color:transparent;" frameBorder="0" src=' + shape.URL + 
  '></iframe>') ) +
  shape.description +   
  '</div></div>';

  google.maps.event.addListener(shape.poiMarker,'click',function(event){
	shape.poiMarker.infowindow=new google.maps.InfoWindow({
    content: drawInfoWindow()
     });
	 shape.poiMarker.infowindow.setContent(contentString);
	 shape.poiMarker.infowindow.setPosition(shape.LLcenter);
	 shape.poiMarker.infowindow.open(map);
  });
  shape.visible=true;
 }else{
 if(shape.visible==true && !forcetoggle){
	shape.poiMarker.setMap(null);
   if(shape.poiMarker.infowindow!=undefined) 
   shape.poiMarker.infowindow.close();
  }else 
  shape.poiMarker.setMap(map);
  shape.visible=!shape.visible;
 } 
 return shape;
}
